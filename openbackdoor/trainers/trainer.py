    ...
    def visualization(self, hidden_states: List, labels: List, poison_labels: List, fig_basepath: Optional[str]="./visualization", fig_title: Optional[str]="vis"):
        ...
        for epoch in tqdm(range(self.epochs+1)):
            ...
            for c in range(num_classes):
                ...
                plt.scatter(embedding.iloc[idx,0], embedding.iloc[idx,1], c=palette[c], s=18, alpha=0.7, marker=markers[c % len(markers)],
                label=f'Class {c}')

            plt.scatter(embedding.iloc[poison_idx,0], embedding.iloc[poison_idx,1], s=30, c='gray', marker='X', alpha=0.9, label='Poison')

            plt.tick_params(labelsize='large')
            plt.legend(fontsize=13, loc='best', frameon=True)
            plt.tight_layout()
            os.makedirs(fig_basepath, exist_ok=True)
            plt.savefig(os.path.join(fig_basepath, f'{fig_title}.png'), dpi=300, bbox_inches='tight')
            plt.savefig(os.path.join(fig_basepath, f'{fig_title}.pdf'), dpi=300, bbox_inches='tight')
